...
function register() {
    $.ajax({
        ...
        success: function (data) {
            alert("The new user was registered with success! You may now login!");
            //alert(JSON.stringify(data));

        },
        error: function (data) {
            alert("There was an issue while registering the new user! Please try again...");
            //alert(JSON.stringify(data));
        }
    });
}
...
function login() {
    ...
    $.ajax({
        ...
        error: function (result) {
            alert("There was a problem with the authentication... you're probably not entering the correct credentials!");
        }
    });
}

/**
 * Override of the Login logic
 */
function CS_login() {
    var body = {
        email: document.getElementById("uEmail").value,
...
function confirmFavTeams() {
    ...
    for (var i = 0; i < selectedTeams.length; i++) {
        if (selectedTeams[i] == "Select your team") {
    ...
    for (var j = 0; j < selectedTeams.length; j++) {
        ...
        $.ajax({
            ...
            error: function (data) {
                alert("Well, something is wrong... please try again! -> " + JSON.stringify(data));
                //alert(JSON.stringify(data));
            }
        });
    }
...
/**
 * Trying to re-write this function with a different logic
 */
function CS_confirmFavTeams() {
    var ts1 = document.getElementById("ts1");
    var ts2 = document.getElementById("ts2");
    var ts3 = document.getElementById("ts3");
