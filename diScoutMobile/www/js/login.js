 ...
 // localStorage.removeItem("favTeams")
  ...
  for (var j = 0; j < selectedTeams.length; j++) {
    ...
    $.ajax({
      ...
      data: body,
      success: function(data) {
        if(!localStorage.getItem("favTeams") || !JSON.parse(localStorage.getItem("favTeams")).includes(data['favTeam'])){
        ...
        if (JSON.parse(localStorage.getItem("favTeams")).length == selectedTeams.length) {
          window.location.replace("chooseTeam.html");
        }
      },
      error: function(data) {
